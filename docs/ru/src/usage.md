...
### Пересоздание декларации (`regenerate`)
```bash
# Предпросмотр плана (ничего не меняет)
ibkr-porez regenerate <id>

# Удалить и пересоздать из сохранённых транзакций
...
Удаляет ошибочную декларацию и пересоздаёт её за тот же период из локально сохранённых транзакций — например, после исправления расчёта или после ввода `assess` по предыдущей декларации, который должен был учесть более поздний PPDG-3R.

Без `--yes` только выводит, что будет удалено, и период для пересоздания. `--force` требуется для удаления не-черновой декларации. Пересоздать можно только самый последний PPDG-3R; декларации PP OPO можно пересоздавать в любое время. Пересозданная декларация возвращается как новый черновик, её нужно снова подать (и, при признанном убытке, снова провести `assess`).
