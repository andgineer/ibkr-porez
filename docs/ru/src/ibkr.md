...
<details>
<summary>Обязательные колонки</summary>
...
## Скачивание XML Flex Query (для `sync --file`)

Если IBKR API (`sync` / `fetch`) временно недоступен, вы можете запустить Flex Query вручную через сайт IBKR:

1. В IBKR перейдите в **Performance & Reports** > **Statements** > **Flex Queries**.
2. Найдите запрос, созданный для `ibkr-porez` (например, `ibkr-porez-data`).
3. Нажмите **Run** (синяя стрелка справа от названия запроса).
4. Выберите формат **XML** и скачайте файл.
5. Используйте файл с командой [`sync --file` ↗](usage.md#синхронизация-из-скачанного-xml-файла-sync---file) или опцией **Sync from Flex Query XML…** в меню ☰ в GUI.
...
4.  Этот файл можно использовать с командой [import ↗](usage.md#импорт-исторических-данных-import).

Импорт CSV поставляет историю покупок для расчёта прироста капитала и больше ничего. Дивиденды, проценты и удержанный налог из него никогда не порождают декларации о доходах (PP OPO) — они формируются только из отчёта Flex Query.
