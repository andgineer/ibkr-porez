> [English](../en/ibkr.html) | [Русский](../ru/ibkr.html) | [Українська](../uk/ibkr.html) | [Srpski](../rs/ibkr.html) | **Српски**
...
<details>
<summary>Обавезне колоне</summary>
...
## Преузимање Flex Query XML-а (за `sync --file`)

Ако IBKR API (`sync` / `fetch`) привремено није доступан, можете покренути Flex Query ручно са IBKR сајта:

1. У IBKR идите на **Performance & Reports** > **Statements** > **Flex Queries**.
2. Пронађите упит који сте креирали за `ibkr-porez` (нпр. `ibkr-porez-data`).
3. Кликните **Run** (плава стрелица десно од назива упита).
4. Изаберите формат **XML** и преузмите фајл.
5. Користите фајл са командом [`sync --file` ↗](usage.md#синхронизација-из-преузетог-xml-фајла-sync---file) или опцијом **Sync from Flex Query XML…** у ☰ менију у GUI-у.
...
4.  Овај фајл се може користити са командом [import ↗](usage.md#увоз-историјских-података-import).

CSV увоз обезбеђује историју куповина за обрачун капиталног добитка и ништа више. Дивиденде, камате и порез по одбитку које носи никада не стварају пријаве о приходу (PP OPO) — оне се генеришу само из Flex Query извештаја.
