> [English](../en/ibkr.html) | [Русский](../ru/ibkr.html) | **Українська** | [Srpski](../rs/ibkr.html) | [Српски](../rs-cyr/ibkr.html)
...
1. У IBKR перейдіть до **Performance & Reports** > **Statements** > **Flex Queries**.
2. Знайдіть запит, створений для `ibkr-porez` (наприклад, `ibkr-porez-data`).
3. Натисніть **Run** (синя стрілка праворуч від назви запиту).
4. Виберіть формат **XML** і завантажте файл.
5. Використайте файл з командою [`sync --file` ↗](usage.md#синхронізація-з-завантаженого-xml-файлу-sync---file) або опцією **Sync from file…** у меню ☰ в GUI.
