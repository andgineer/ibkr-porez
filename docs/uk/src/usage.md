...
### Повторне створення декларації (`regenerate`)
```bash
# Попередній перегляд плану (нічого не змінює)
ibkr-porez regenerate <id>

# Видалити та створити заново зі збережених транзакцій
...
Видаляє помилкову декларацію та повторно створює її за той самий період з локально збережених транзакцій — наприклад, після виправлення розрахунку або після введення `assess` за попередньою декларацією, яку мав врахувати пізніший PPDG-3R.

Без `--yes` лише виводить, що буде видалено, і період для повторного створення. `--force` потрібен для видалення не-чернеткової декларації. Повторно створити можна лише найновіший PPDG-3R; декларації PP OPO можна створювати повторно будь-коли. Повторно створена декларація повертається як нова чернетка, її потрібно знову подати (і, за наявності визнаного збитку, знову провести `assess`).
